...
class Generator(nn.Module):
    ...
    def forward(
        ...
        out = self.conv1(out, styles[0].float() if is_stylespace else latent[:, 0], noise=noise[0],
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            out = conv1(out, styles[i].float() if is_stylespace else latent[:, i], noise=noise1,
                        is_stylespace=is_stylespace)
            outs.append(out)

            out = insert_feature(out, i + 1)
            out = conv2(out, styles[i + 1].float() if is_stylespace else latent[:, i + 1], noise=noise2,
                        is_stylespace=is_stylespace)
            outs.append(out)

            skip = to_rgb(out, to_rgb_stylespace[i // 2 + 1].float() if is_stylespace else latent[:, i + 2], skip,
                          is_stylespace=is_stylespace)
